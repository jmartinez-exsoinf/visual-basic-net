...
```VisualBasic
❌ Vulnerable
Dim sql As String = "SELECT * FROM usuarios WHERE nombre = '" & nombreUsuario & "'"
```

En su lugar:
```VisualBasic
✅ Seguro
Using cmd As New NpgsqlCommand("SELECT * FROM usuarios WHERE nombre = @nombre", conn)
...
Para números:
```VisualBasic
If Not Integer.TryParse(idUsuario, Nothing) Then
...
```VisualBasic
Private Function ValidarTexto(input As String) As String
...
```VisualBasic
Public Async Function BuscarUsuario(nombreUsuario As String) As Task(Of IActionResult)
